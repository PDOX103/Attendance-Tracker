...
const Create_Session = () => {
  ...
  const formSubmit = async (data) => {
    ...
    try {
      ...
      if (res.status === 409) {
        toast.error("Session already exists!", { position: "bottom-center", autoClose: 1000 });
      } else if (res.ok) {
        toast.success("Session Created Successfully!", { position: "bottom-center", autoClose: 1000 });
      ...
      toast.error("Failed to create session!", { position: "bottom-center", autoClose: 1000 });
    }
  };
    ...
    <div className="flex">
      ...
      <div className="flex-1 p-6">
        <div className="bg-BLUE shadow-lg rounded-lg p-6 max-w-lg mx-auto">
          <h2 className="text-lg font-semibold mb-4 text-center font-roboto">Create Session</h2>
          <form onSubmit={handleSubmit(formSubmit)} className="space-y-4">
            
            {/* Date */}
            <div>
              <label className="block text-sm font-roboto font-medium mb-1">Date</label>
              <input
                type="date"
            ...
            {/* Start Time (HH:MM:SS) */}
            <div>
              <label className="block text-sm font-roboto font-medium mb-1">Start Time (HH:MM:SS)</label>
              <input
                type="time"
                ...
                className="w-full font-roboto border rounded-md p-2"
              />
              {errors.start_time && <p className="text-red-500 text-xs">Start time is required.</p>}
            </div>

            {/* End Time (HH:MM:SS) */}
            <div>
              <label className="block text-sm font-roboto font-medium mb-1">End Time (HH:MM:SS)</label>
              <input
                type="time"
                step="1" // Allows seconds in input
                ...
                className="w-full font-roboto border rounded-md p-2"
              />
              {errors.end_time && <p className="text-red-500 text-xs">End time is required.</p>}
            </div>
